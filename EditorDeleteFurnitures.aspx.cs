...
public partial class EditorDeleteFurnitures : System.Web.UI.Page
{
    ...
    private void BindData()
    {
        try
        {
            ...
            SqlDataAdapter da = new SqlDataAdapter("SELECT id,product,descp,price FROM furniture where mail='" + mailTxtBox.Text + "' order by createdOn desc", con);
    ...
    protected void GridView1_RowUpdating(object sender, GridViewUpdateEventArgs e)
    {
        try
        {
            ...
            {
                if (serverSideValidation(((TextBox)GridView1.Rows[e.RowIndex].Cells[3].Controls[0]).Text.Trim(), ((TextBox)GridView1.Rows[e.RowIndex].Cells[4].Controls[0]).Text.Trim()))
                {
                    SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["uscresellall"].ConnectionString);
                    SqlCommand cmd = new SqlCommand();
                    cmd.CommandText = "UPDATE furniture SET descp=@cat_name,price=@price,createdOn='" + DateTime.Now + "' WHERE id=@cat_id";
                    cmd.Parameters.Add("@cat_name", SqlDbType.VarChar).Value = ((TextBox)GridView1.Rows[e.RowIndex].Cells[3].Controls[0]).Text.Trim();
                    cmd.Parameters.Add("@cat_id", SqlDbType.Int).Value = Convert.ToInt32(GridView1.Rows[e.RowIndex].Cells[1].Text.Trim());
                    cmd.Parameters.Add("@price", SqlDbType.Float).Value = Convert.ToDouble(((TextBox)GridView1.Rows[e.RowIndex].Cells[4].Controls[0]).Text.Trim());
                    cmd.Connection = con;
                    con.Open();
                    cmd.ExecuteNonQuery();
                    con.Close();
    ...
    protected void GridView1_RowDeleting(object sender, GridViewDeleteEventArgs e)
    {
        try
        {
            ...
            SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["uscresellall"].ConnectionString);
            SqlCommand cmd = new SqlCommand();
            cmd.CommandText = "DELETE FROM furniture WHERE id=@cat_id";
