...
public partial class Furnitures : System.Web.UI.Page
{
    int captchaResult;
    ...
    protected void BTN_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (codeSent == TxtCode.Text.Trim())
            {
                ...
                INcommand1.CommandText = "insert into housing (name,mail,phonenumer,Lookingfor,descp,gender,createdOn) values(@name,@email,@phoneno,@lookingfor,@descp,@gender,@createdOn)";
                ...
                INcommand1.Parameters.AddWithValue("@lookingfor", watTextBox.Text.Trim());
                INcommand1.Parameters.AddWithValue("@descp", descriptionTextBox.Text.Trim());
                INcommand1.Parameters.AddWithValue("@gender", expectedTextBox.Text.Trim());
    ...
    private bool serverSideValidation()
    {
        try
        {
            ...
            string products = watTextBox.Text.Trim();
            int isprob = 0;
            int nameempty = 0;
            int nameemptyProd = 0;
            ...
            if (products == null || products == string.Empty || products == " " || !(regexname.IsMatch(products)))
            {
                watLabel.Text = "Enter valid product name,no special characters are allowed.";
                isprob = 1;
                nameemptyProd = 1;
            }
            else
            {
                watLabel.Text = string.Empty;
            }

            if (nameemptyProd == 0)
            {
                if (products.Length == 20 && isprob != 1)
                {
                    watLabel.Text = "Field can't be greater than 20";
                    isprob = 1;
                }
                else
                {
